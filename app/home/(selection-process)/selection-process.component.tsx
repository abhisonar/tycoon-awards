...
const SelectionProcessComponent = () => {
  const [upcomingEvent, setUpcomingEvent] = useState<IEvent | undefined>(
    undefined
  );
  ...
  return (
    <div className="flex flex-col mt-4 items-center gap-4">
      ...
      <div className="flex flex-col md:flex-row items-center justify-center flex-wrap w-full p-3 gap-3">
        {SELECTION_PROCESS_DATA.map((item) => (
          <div
            key={item.title}
            className="flex flex-col p-3 bg-white w-full rounded-md md:basis-[45%]  lg:basis-[30%] shadow-md"
          >
